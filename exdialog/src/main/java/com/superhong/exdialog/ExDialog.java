...
import com.superhong.exdialog.util.DialogUtils;
...
public class ExDialog extends Dialog implements View.OnClickListener,
    ...
    private void setContentView() {
        ...
        switch (mBuilder.mTypeEnum) {
            ...
            case SHEET:
                ...
                break;
            case FULL_SCREEN:
                mRootView = (LinearLayout) inflater.inflate(R.layout.exd_dialog_full_screen,null);
                ((FrameLayout) mRootView.findViewById(R.id.custom_container)).addView(mBuilder.mCustomView);
                break;
            case DIALOG:
            default:
                if (mBuilder.mCustomView != null) {
                    ...
                    if (mBuilder.mWithFootView) {
                        mRootView =
                                (MaxHeightLayout) inflater.inflate(R.layout.exd_dialog_custom_with_foot,null);
                        FrameLayout customContainer =
                                (FrameLayout) mRootView.findViewById(R.id.custom_container);
                        customContainer.addView(mBuilder.mCustomView);
                    } else {
                        mRootView = (MaxHeightLayout) inflater.inflate(R.layout.exd_dialog_custom
                                ,null);
    ...
    private void bindDialogView() {
        ...
        if (mBuilder.mInputSet) {
            ...
            if (!TextUtils.isEmpty(mBuilder.mInputText)) {
                mExdInput.setText(mBuilder.mInputText);
                mExdInput.setSelection(mExdInput.getText().length());
            }
            if (mBuilder.mInputType != -1) {
                mExdInput.setInputType(mBuilder.mInputType);
            }
    ...
    private void initFootView() {
        ...
        if (mBuilder.mSingleAction) {
            ...
            mExdCancel.setVisibility(View.VISIBLE);
            mExdLineVertical.setVisibility(View.VISIBLE);
        ...
        mExdConfirm.getViewTreeObserver().addOnGlobalLayoutListener(
                new ViewTreeObserver.OnGlobalLayoutListener() {
                    ...
                    public void onGlobalLayout() {
                        ...
                        // 判断 取消和确认按钮需要换行时 改变布局成纵向
                        if (!mBuilder.mSingleAction && (mExdConfirm.getLineCount() > 1 || mExdCancel.getLineCount() > 1)) {
                            LinearLayout parent = (LinearLayout) mExdConfirm.getParent();
                            parent.setOrientation(LinearLayout.VERTICAL);
                            LinearLayout.LayoutParams param =
                                    (LinearLayout.LayoutParams) mExdLineVertical.getLayoutParams();
                            param.weight = 0;
                            param.width = ViewGroup.LayoutParams.MATCH_PARENT;
                            param.height = 2;
                            LinearLayout.LayoutParams param1 =
                                    (LinearLayout.LayoutParams) mExdConfirm.getLayoutParams();
                            param1.weight = 0;
                            param1.width = ViewGroup.LayoutParams.MATCH_PARENT;
                            LinearLayout.LayoutParams param2 =
                                    (LinearLayout.LayoutParams) mExdCancel.getLayoutParams();
                            param2.weight = 0;
    ...
    private void bindListView() {
        ...
        if (TextUtils.isEmpty(mBuilder.content)) {
            mExdContent.setVisibility(View.GONE);
        } else {
            mExdContent.setVisibility(View.VISIBLE);
            mExdContent.setText(mBuilder.content);
        }
        if (mBuilder.mTitleColor != -1) {
            mExdTitle.setTextColor(mBuilder.mTitleColor);
        }
                    ...
                    public void onGlobalLayout() {
                        ...
                        if (mRootView.getHeight() + outLocation1[1] > bottom) {
                            LinearLayout.LayoutParams layoutParams =
                                    (LinearLayout.LayoutParams) mTvSheetCancel
                                    .getLayoutParams();
    ...
    private void initWindow() {
        ...
        if (mBuilder.mTypeEnum == TypeEnum.SHEET || mBuilder.mTypeEnum == TypeEnum.FULL_SCREEN) {
            window.setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        }
        ...
        } else if (mBuilder.mTypeEnum == TypeEnum.FULL_SCREEN) {
            lp.width = windowWidth;
            lp.height = windowHeight;
        } else {
    ...
    public static class Builder {
        ...
        private void init(Context context) {
            ...
            if (background == null) {
                background = context.getResources().getDrawable(R.drawable.exd_shape_radius_12);
            }
            ...
            this.mPositiveTextColor =
                    a.getColorStateList(R.styleable.ExDialog_exdPositiveTextSelector);
            this.mNegativeTextColor =
                    a.getColorStateList(R.styleable.ExDialog_exdNegativeTextSelector);
        ...
        public Builder inputHint(@Nullable CharSequence hint) {
            this.mInputSet = true;
            this.mInputHint = hint;
            return this;
        }

        public Builder inputText(@Nullable CharSequence text) {
            this.mInputSet = true;
            this.mInputText = text;
            return this;
        }

        public Builder inputCallback(InputCallback callback) {
            this.mInputCallback = callback;
            return this;
        }

        public Builder input(@StringRes int hint,InputCallback callback) {
            return input(hint == 0 ? null : mContext.getText(hint),callback);
        }

        public Builder input(@Nullable CharSequence hint,InputCallback callback) {
            this.mInputSet = true;
            this.mInputCallback = callback;
            this.mInputHint = hint;
            return this;
        ...
        public Builder inputTypeTextPassWord() {
            this.mInputType = InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD;
            return this;
        }
        ...
        public Builder itemSelectorAttr(@AttrRes int colorAttr) {
            return itemSelector(DialogUtils.resolveActionTextColorStateList(this.mContext,
                    colorAttr,null));
        }
        ...
        public Builder selectPosition(int selectPosition) {
            this.mSelectPosition = selectPosition;
            return this;
        }

        public Builder selectIcon(@DrawableRes int res) {
            selectIcon(mContext.getResources().getDrawable(res));
            return this;
        }

        public Builder selectIcon(Drawable res) {
            this.selectIcon = res;
            return this;
        }
